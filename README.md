...
- **Known gap:** `schema_validator.py`'s unknown-field reporting calls the GitHub Issues API, but `modules/lambda_provisioning`'s IAM role only grants `ssm:GetParameter` for the Okta token, not a GitHub token — that path is real and tested in isolation, but would fail with a missing env var/`AccessDenied` if actually deployed today.
